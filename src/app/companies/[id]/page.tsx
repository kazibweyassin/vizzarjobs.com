...
export default function CompanyPage() {
  ...
  const { data: jobs = [], isLoading: isLoadingJobs } = 
    api.jobs.getByCompany.useQuery({ companyId: id }, { enabled: !!id });

  useEffect(() => {
    // Set page metadata dynamically
    if (company) {
      document.title = `${company.name} | VizzarJobs`;
    }
  }, [company]);
  ...
  return (
    <div className="container py-8">
      <div className="bg-white rounded-xl overflow-hidden shadow-md mb-8">
        ...
        <div className="px-8 pb-8 relative">
          <div className="flex flex-col md:flex-row md:items-end -mt-16 mb-6">
            <div className="w-32 h-32 rounded-xl bg-gray-200 border-4 border-white"></div>
            <div className="mt-4 md:mt-0 md:ml-6 flex-1">
